...
void BasicEKF::init_state(LeggedState& state) {
    ...
    for (int i = 0; i < NUM_LEG; ++i) {
        Eigen::Vector3d fk_pos = state.fbk.foot_pos_rel.block<3, 1>(0, i);
        x.segment<3>(6 + i * 3) = state.fbk.root_rot_mat * fk_pos + x.segment<3>(0);
    }
}
...
void BasicEKF::update_estimation(LeggedState& state, double dt) {
    ...
    Eigen::Vector3d u = state.fbk.root_rot_mat * state.fbk.imu_acc + Eigen::Vector3d(0, 0, -9.81);
    ...
    if (state.ctrl.movement_mode == 0) {  // stand
        ...
        for (int i = 0; i < NUM_LEG; ++i) {
            ...
            // estimated_contacts[i] = 1.0/(1.0+std::exp(-(state.fbk.foot_force(i)-20)));
...
//    r((i-1)*3+1:(i-1)*3+3) = body_v - R_er*leg_v;
    ...
    for (int i=0; i<NUM_LEG; ++i) {
        Eigen::Vector3d fk_pos = state.fbk.foot_pos_rel.block<3,1>(0,i);
        y.block<3,1>(i*3,0) = state.fbk.root_rot_mat*fk_pos;   // fk estimation
        Eigen::Vector3d leg_v = -state.fbk.foot_vel_rel.block<3,1>(0,i) - Utils::skew(state.fbk.imu_ang_vel)*fk_pos;
        y.block<3,1>(NUM_LEG*3+i*3,0) =
                (1.0-estimated_contacts[i])*x.segment<3>(3) +  estimated_contacts[i]*state.fbk.root_rot_mat*leg_v;      // vel estimation
    ...
    for (int i = 0; i < NUM_LEG; ++i) {
        if (estimated_contacts[i] < 0.5) {
            state.fbk.estimated_contacts[i] = false;
        } else {
            state.fbk.estimated_contacts[i] = true;
        }
    }
