...
LciMpc::LciMpc() {
    ...
    jl_eval_string("include(\"scripts/centroidalStand_test.jl\")");
    jl_eval_string("include(\"scripts/centroidalTrot_test.jl\")");
...
bool LciMpc::update(LeggedState &legged_state, double t, double dt) {
    ...
    x.segment<4>(36) << legged_state.fbk.foot_force; 
    ...
    for(size_t i = 0; i < NUM_LEG; ++i) {
        // this estimated_contacts is generated by the EKF
    ...
    for(int i = 0; i < NUM_LEG; ++i) {
        ...
        foot_forces_grf.block<3,1>(0,i) = legged_state.fbk.root_rot_mat.transpose() * foot_forces.segment<3>(i*3); 
