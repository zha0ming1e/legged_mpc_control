...
LciMpc::LciMpc() {
    ...
    for (int i = 0; i < 18; ++i) {
        pos_filter_bank[i] = MovingWindowFilter(2);
        vel_filter_bank[i] = MovingWindowFilter(2);
    }
}
...
bool LciMpc::update(LeggedState &legged_state, double t, double dt) {
    ...
    for (int i = 0; i < NUM_LEG; ++i) {
        if (legged_state.fbk.foot_contact_flag[i] < 0.5) {
            legged_state.ctrl.plan_contacts[i] = false;
        } else {
            legged_state.ctrl.plan_contacts[i] = true;
        }
    }
